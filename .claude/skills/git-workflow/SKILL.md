...
5. **Gates** (required, run locally): `pnpm check` = `pnpm lint`, `pnpm typecheck`, `pnpm test:coverage` (thresholds in test-coverage skill), `cargo fmt --check`, `cargo clippy -D warnings`, `cargo test`, `pnpm tauri build --debug --no-bundle`. `pnpm install` wires `pnpm check:fast` (everything but the Tauri build) into a pre-push hook; `SKIP_CHECKS=1 git push` bypasses it. GitHub Actions runs the same suite only on pushes to `main` (macOS minutes bill at 10x) — never wait on it for a PR.
