...
it("applyQuality calls set_preview_quality and records whether a regen is applicable", async () => {
  mockIPC((cmd) => {
    if (cmd === "get_settings") return { preview_edge: 2000 };
    if (cmd === "storage_usage") {
      return { thumbs_400_bytes: 0, previews_bytes: 0, catalog_bytes: 0, total_bytes: 0, file_count: 0 };
...
it("regenApplicable stays false when set_preview_quality reports no regen is applicable", async () => {
  mockIPC((cmd) => {
    if (cmd === "get_settings") return { preview_edge: 800 };
    if (cmd === "storage_usage") {
      return { thumbs_400_bytes: 0, previews_bytes: 0, catalog_bytes: 0, total_bytes: 0, file_count: 0 };
    }
    if (cmd === "set_preview_quality") return false;
    return undefined;
  });
  ...
  act(() => result.current.applyQuality(2000));
...
it("clears regenApplicable once a running regen sweep finishes", async () => {
  mockIPC((cmd) => {
    if (cmd === "get_settings") return { preview_edge: 2000 };
    if (cmd === "storage_usage") {
      return { thumbs_400_bytes: 0, previews_bytes: 0, catalog_bytes: 0, total_bytes: 0, file_count: 0 };
  ...
  await waitFor(() => expect(result.current.regenApplicable).toBe(false));
});
