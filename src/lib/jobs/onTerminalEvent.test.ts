...
it("invalidates media, media-count, jobs, unorganized, drives, and scan-error-count on finished", () => {
  const { queryClient, invalidateSpy } = client();
  onTerminalEvent({ kind: "finished", job_id: "scan-0", ok: 9, failed: 0, skipped: 1 }, queryClient, "Scan");

  for (const key of ["media", "media-count", "jobs", "unorganized", "drives", "scan-error-count"]) {
